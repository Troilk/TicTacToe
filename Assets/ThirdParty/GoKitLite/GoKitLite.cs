...
namespace Prime31.GoKitLite
{
	public class GoKitLite : MonoBehaviour
	{
		...
		public class Tween
		{
			...
			internal Action<Transform> onDelayComplete;
			...
			internal void PerformAction(float easedTime)
			{
			...
			{
				// HACK: workaround for corrupted or weirdly set tweens
				if(duration <= 0.0f)
				{
					PerformAction(1.0f);
					return true;
			...
			private void handleLooping()
			{
				...
				if( loopType == GoKitLite.LoopType.RestartFromBeginning || !isRunningInReverse )
				{
			...
			public Tween setDelayCompletionHandler(Action<Transform> onDelayComplete)
			{
				this.onDelayComplete = onDelayComplete;
				return this;
			}
			...
			public Tween setLoopType( LoopType loopType, int loops = 1, float delayBetweenLoops = 0f )
			{
				...
				{
					// avoid overflows for infinite loops
					loops = Math.Min(loops, int.MaxValue / 2);
					loops = loops * 2 - 1;
				}
				this.loops = loops;
			...
			public Tween setTimeScaleIndependent(bool independent = true)
			{
		...
		private void Update()
		{
			...
			for( var i = _activeTweens.Count - 1; i >= 0; --i )
			{
				...
				if( tween.transform == null || tween.tick() )
				{
					...
					// HACK: i may be invalid in case onComplete callback stopped another tween (so list has shifted) OR this tween itself might have been stopped
					i = _activeTweens.IndexOf(tween);
					if(i != -1)
						removeTween( tween, i );
				}
			}
		...
		public Tween customAction( Transform trans, float duration, Action<Transform,float> action )
		{
        ...
        /// </summary>
        public bool setTweenPauseState( int id, bool isPaused )
        {
            for( var i = 0; i < _activeTweens.Count; i++ )
            {
                if( _activeTweens[i].id == id )
                {
                    _activeTweens[i].isPaused = isPaused;
                    return true;
                }
            }
        ...
        /// </summary>
        public void setAllTweenPauseState( bool isPaused )
        {
            for( var i = 0; i < _activeTweens.Count; i++ )
                _activeTweens[i].isPaused = isPaused;
        }
